 ...
 * - Ride service chỉ cần đọc `x-user-id` và `x-user-role` từ request.
 * - Tránh lặp lại việc giải mã JWT.
 *
 * Test Case TC91: Request không có token → 401
 * Test Case TC92: Token bị sửa payload → 401 (handled by gateway)
 * Test Case TC95: User thường gọi API admin → 403
 * Test Case TC96: Driver cố truy cập thông tin user khác → 403
 */
 ...
 * Test Case TC95: User role không gọi được endpoint admin
